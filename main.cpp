...
vector<string> generateCandidates(string& ciphertext) {
    ...
    for (int j = 0; j < 1000; j++) {
        string candidate = "";
        for (int i=0; i < 50; i++) {
            candidate += DICTIONARY2[rand() % DICTIONARY2.size()];
            candidate += " ";
        }
...
void test2guess( string& ciphertext) {
    ...
    candidates.push_back(ciphertext);
    ...
    for (int i=0; i < allOccurrences.size(); i++) {
        double x_i = 0;
        for (auto it_o = allOccurrences[i].begin(), it_e = allOccurrences[allOccurrences.size()-1].begin(), end_e = allOccurrences[allOccurrences.size()-1].end(), end_o = allOccurrences[i].end();
             it_o != end_o;)
        {
            //chi squared calculation
            double exp = it_e->second;
